...
#include <iostream>
AssetManagerEffectPanel::AssetManagerEffectPanel(const int width, const int height):
PropertiesPanel(width, height)
{
    //ctor
   // getRenderer()->setBackgroundColor(sf::Color::Blue);
    this->appendSpacer();
    pathBox = this->appendEditBox("Path", "");
    this->appendEditBox("Tags", "");
}

void AssetManagerEffectPanel::save(const std::vector<boost::filesystem::path> &outputDirectory)
{
...
AssetManagerEffectPanel::~AssetManagerEffectPanel()
{
    //dtor
}
